...
- `application.yml` - default config file used for local development
- `config.yaml` - template of external config

Environment-specific properties for `dev`, `test` and `prod` profiles are stored in the deployment configuration (disaster-ninja-cd repository).
